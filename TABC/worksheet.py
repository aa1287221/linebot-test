...
from pygsheets.exceptions import (
    CellNotFound, InvalidArgumentValue, RangeNotFound)
...
def index(self, index):
    ...
    if self._linked:
        self.client.sheet.update_sheet_properties_request(
            self.spreadsheet.id, self.jsonSheet['properties'], 'index')
...
def title(self, title):
    ...
    if self._linked:
        self.client.sheet.update_sheet_properties_request(
            self.spreadsheet.id, self.jsonSheet['properties'], 'title')
...
def hidden(self, hidden):
    ...
    if self._linked:
        self.client.sheet.update_sheet_properties_request(
            self.spreadsheet.id, self.jsonSheet['properties'], 'hidden')
...
def rows(self, row_count):
    if row_count == self.rows:
        return
    self.jsonSheet['properties']['gridProperties']['rowCount'] = int(
        row_count)
    if self._linked:
...
def cols(self, col_count):
    if col_count == self.cols:
        return
    self.jsonSheet['properties']['gridProperties']['columnCount'] = int(
        col_count)
    if self._linked:
        self.client.sheet.update_sheet_properties_request(self.spreadsheet.id, self.jsonSheet['properties'],
...
def frozen_rows(self, row_count):
    self.jsonSheet['properties']['gridProperties']['frozenRowCount'] = int(
        row_count)
    if self._linked:
        self.client.sheet.update_sheet_properties_request(self.spreadsheet.id, self.jsonSheet['properties'],
...
def frozen_cols(self, col_count):
    self.jsonSheet['properties']['gridProperties']['frozenColumnCount'] = int(
        col_count)
    if self._linked:
        self.client.sheet.update_sheet_properties_request(self.spreadsheet.id, self.jsonSheet['properties'],
...
def _update_grid(self, force=False):
    ...
    if not self.data_grid or force:
        self.data_grid = self.get_all_values(
            returnas='cells', include_tailing_empty=True, include_tailing_empty_rows=True)
    elif not force:
        updated = datetime.datetime.strptime(
            self.spreadsheet.updated, '%Y-%m-%dT%H:%M:%S.%fZ')
...
def link(self, syncToCloud=True):
    ...
    if syncToCloud:
        self.client.sheet.update_sheet_properties_request(
            self.spreadsheet.id, self.jsonSheet['properties'], '*')
    else:
        wks = self.spreadsheet.worksheet(property='id', value=self.id)
        self.jsonSheet = wks.jsonSheet
    # flatten the list
    tmp_data_grid = [
        item for sublist in self.data_grid for item in sublist]
...
def cell(self, addr):
    ...
    try:
        if type(addr) is str:
            val = self.client.get_range(
                self.spreadsheet.id, self._get_range(addr, addr), 'ROWS')[0][0]
        elif type(addr) is tuple:
            label = format_addr(addr, 'label')
            val = self.client.get_range(
                self.spreadsheet.id, self._get_range(label, label), 'ROWS')[0][0]
        else:
            raise CellNotFound
...
def get_values(self, start, end, returnas='matrix', majdim='ROWS', include_tailing_empty=True,
    ...
    :returns: 'range':   :class:`DataRange <DataRange>`
             ...
             'matrix':  [[ ... ], [ ... ], ...]
    """
    include_tailing_empty = kwargs.get(
        'include_empty', include_tailing_empty)
    include_tailing_empty_rows = kwargs.get(
        'include_all', include_tailing_empty_rows)
    ...
    for key in kwargs:
        if key in _deprecated_keywords:
            warnings.warn(
                'The argument {} is deprecated. Use {} instead.'.format(key, _deprecated_keyword_mapping[key]), category=DeprecationWarning)
            kwargs.pop(key, None)
    ...
    # append empty rows in end
    if include_tailing_empty_rows and (max_rows - len(values)) > 0:
        values.extend([[]] * (max_rows - len(values)))
    if include_tailing_empty:  # append tailing cells in rows
        values = [list(x + [empty_value] * (max_cols - len(x)))
                  for x in values]
    ...
    if returnas == 'matrix':
        ...
        for k in range(len(values)):
            ...
            for i in range(len(values[k])):
                if majdim == "ROWS":
                    cells[-1].append(Cell(pos=(start[0] + k, start[1] + i),
                                          worksheet=self, cell_data=values[k][i]))
                else:
                    cells[-1].append(Cell(pos=(start[0] + i, start[1] + k),
                                          worksheet=self, cell_data=values[k][i]))
        ...
        if returnas.startswith('cell'):
            ...
            dd = DataRange(start, format_addr(end, 'label'),
                           worksheet=self, data=cells)
            return dd
...
def update_cell(self, **kwargs):
    warnings.warn(_warning_mesage.format(
        "method", "update_value"), category=DeprecationWarning)
...
def update_values(self, crange=None, values=None, cell_list=None, extend=False, majordim='ROWS', parse=None):
    ...
    if cell_list:
        if type(cell_list[0]) is list:
            ...
            values = [[None for x in range(self.cols)]
            ...
            values = [row[min_tuple[1] - 1:max_tuple[1]]
                      for row in values[min_tuple[0] - 1:max_tuple[0]]]
            crange = str(format_addr(tuple(min_tuple))) + \
                ':' + str(format_addr(tuple(max_tuple)))
        ...
        raise InvalidArgumentValue(
            "provide either cells or values, not both")
    ...
    if estimate_size:
        ...
        if majordim == 'ROWS':
            end_r_tuple = (
                start_r_tuple[0] + len(values), start_r_tuple[1] + max_2nd_dim)
        else:
            end_r_tuple = (
                start_r_tuple[0] + max_2nd_dim, start_r_tuple[1] + len(values))
...
def update_cells_prop(self, **kwargs):
    warnings.warn(_warning_mesage.format(
        'method', 'update_cells'), category=DeprecationWarning)
...
def resize(self, rows=None, cols=None):
    ...
    try:
        ...
        self.logger.error("couldnt resize the sheet to " +
                          str(rows) + ',' + str(cols))
...
def clear(self, start='A1', end=None, fields="userEnteredValue"):
    ...
    request = {"updateCells": {"range": self._get_range(
        start, end, "GridRange"), "fields": fields}}
    self.client.sheet.batch_update(self.spreadsheet.id, request)
...
def find(self, pattern, searchByRegex=False, matchCase=False, matchEntireCell=False, includeFormulas=False,
    ...
    if rows:
        cells = self.data_grid[rows[0] - 1: rows[1]]
    found_cells = []
    for cells_row in cells:
        if cols:
            cells_row = cells_row[cols[0] - 1: cols[1]]
...
def create_named_range(self, name, start, end, returnas='range'):
    ...
    res = self.client.sheet.batch_update(self.spreadsheet.id, request)[
        'replies'][0]['addNamedRange']['namedRange']
    if returnas == 'json':
        return res
    else:
...
def get_named_range(self, name):
    ...
    nrange = [x for x in self.spreadsheet.named_ranges if x.name ==
              name and x.worksheet.id == self.id]
    if len(nrange) == 0:
...
def get_named_ranges(self, name=''):
    ...
    if name == '':
        self.spreadsheet.update_properties()
        nrange = [
            x for x in self.spreadsheet.named_ranges if x.worksheet.id == self.id]
        return nrange
    else:
...
def delete_named_range(self, name, range_id=''):
    ...
    self.spreadsheet._named_ranges = [
        x for x in self.spreadsheet._named_ranges if x["namedRangeId"] != range_id]
...
def set_dataframe(self, df, start, copy_index=False, copy_head=True, fit=False, escape_formulae=False, **kwargs):
    ...
    if copy_head:
        ...
        if isinstance(df.columns, pd.MultiIndex):
            # skip index columns
            head = [""] * num_indexes if copy_index else []
...
def get_as_df(self, has_header=True, index_colum=None, start=None, end=None, numerize=True,
    ...
    if numerize:
        values = [numericise_all(
            row[:len(values[0])], empty_value) for row in values]
...
def export(self, file_format=ExportType.CSV, filename=None, path=''):
    ...
    if not self._linked:
        return
    self.client.drive.export(
        self, file_format=file_format, filename=filename, path=path)
...
def copy_to(self, spreadsheet_id):
    ...
    response = self.client.sheet.sheets_copy_to(
        self.spreadsheet.id, self.id, spreadsheet_id)
...
def get_charts(self, title=None):
    ...
    """
    matched_charts = []
    chart_data = self.client.sheet.get(
        self.spreadsheet.id, fields='sheets(charts,properties/sheetId)')
    sheet_list = chart_data.get('sheets')
    sheet = [x for x in sheet_list if x.get(
        'properties', {}).get('sheetId') == self.id][0]
...
def __iter__(self):
    rows = self.get_all_values(
        majdim='ROWS', include_tailing_empty=False, include_tailing_empty_rows=False)
    for row in rows:
